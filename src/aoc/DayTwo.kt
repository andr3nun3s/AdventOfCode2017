...
fun main(args: Array<String>) {
    val input = "116\t1259\t1045\t679\t1334\t157\t277\t1217\t218\t641\t1089\t136\t247\t1195\t239\t834\n" +
    ...
    println("Part one solution: ${calculateChecksum(input, ::differenceOfMaxMin)}")
    println("Part two solution: ${calculateChecksum(input, ::differenceOfDivisors)}")
}

fun calculateChecksum(input: String, algorithm: KFunction1<List<Int>, Int>): Int =
        input.split("\n")
                .map { line ->
                    line.split("\t")
                            .asIterable()
...
private fun differenceOfMaxMin(list: List<Int>) = Collections.max(list) - Collections.min(list)

private fun differenceOfDivisors(list: List<Int>): Int {
    val pair = findPair(list, BiPredicate({ a, b -> a % b == 0 }))
    return pair.first / pair.second
}

/*
    Stack Overflow:
    https://stackoverflow.com/questions/25336165/use-java-8-lambdas-to-find-a-pair-of-objects-in-a-list-that-meet-a-certain-crite
 */
fun findPair(list: List<Int>, predicate: BiPredicate<Int, Int>): Pair<Int, Int> {
    val item = list.parallelStream()
            .flatMap { item1 ->
                list.stream()
